...
OpenAPI 3.1 契约校验、OpenTelemetry、Prometheus；Next.js App Router 与 pnpm；K3s、
...
Next.js 组件/集成测试；双班级隔离、三种入口、CPU/NPU、终端采集、培训期结束与长期任务的端到端验收。
...
W3C Trace Context、审计、`/healthz` 与 `/readyz` 为必需。
...
*Pre-Phase 0: PASS. Post-Phase 1: PASS. 未记录宪法例外。*
...
5. 冻结 OpenAPI；按 DDD read model 更新每页/每个弹窗的前端原型，再并行实现工作区门户与教师终端过程视图。
